...
describes what users get. Pick a format with `-o`, or let climatemeteo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
climatemeteo <command> -o table   # aligned columns for reading
climatemeteo <command> -o jsonl   # one JSON object per line, for piping
climatemeteo <command> -o json    # a single JSON array
climatemeteo <command> -o csv     # spreadsheet friendly
climatemeteo <command> -o tsv     # tab-separated
climatemeteo <command> -o url     # just the URL column
climatemeteo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
climatemeteo <command> --fields id,title,url
```
...
```bash
climatemeteo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
climatemeteo <command>            # a table, because this is a terminal
climatemeteo <command> | wc -l    # JSONL, because this is a pipe
```
